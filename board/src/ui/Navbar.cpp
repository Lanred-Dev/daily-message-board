...
void Navbar::setup()
{
    ...
    CircleButton *messagePageButton = new CircleButton(Vector2(startX, centerY), buttonRadius, 10, Color::PRIMARY_BUTTON);
    messagePageButton->onPress.connect([]() { UI::setCurrentPage(0); });
    UI::addInstance(messagePageButton);

    CircleButton *imagePageButton = new CircleButton(Vector2(startX + buttonDiameter + buttonSpacing, centerY), buttonRadius, 10, Color::PRIMARY_BUTTON);
    imagePageButton->onPress.connect([]() { UI::setCurrentPage(1); });
    UI::addInstance(imagePageButton);

    CircleButton *calendarPageButton = new CircleButton(Vector2(startX + 2 * (buttonDiameter + buttonSpacing), centerY), buttonRadius, 10, Color::PRIMARY_BUTTON);
